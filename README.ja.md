...
[Blueprin](blueprin-app.vercel.app) 用の **プラグイン**、**コネクタ**、**拡張機能**、**統合** を構築するための公式SDK — インドネシアの建設業向けプロフェッショナル建築予算プラットフォーム。
...
export default definePlugin({
  ...
  name: '私の最初のプラグイン',
  version: '1.0.0',
  description: 'Blueprin 用の最初のプラグイン',

  activate(ctx) {
    // イベントをリッスン
    ctx.events.on('blueprin:project:created', (data) => {
      console.log('プロジェクトが作成されました:', data.project.name);
    });
...
- [GitHub Discussions](https://github.com/qalvinahmad/blueprin-sdk/discussions) — 質問をしたり、アイデアを共有したり、他の開発者とつながりましょう
- [Issue Tracker](https://github.com/qalvinahmad/blueprin-sdk/issues) — バグを報告し、機能をリクエスト

## ドキュメント

- [完全なドキュメント](https://blueprin-docs.vercel.app) — 完全なガイド、APIリファレンス、例
