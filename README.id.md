...
[![Bahasa Indonesia](https://img.shields.io/badge/Bahasa_Indonesia-FF0000?style=flat&label=You%20are%20here)](README.id.md)
...
SDK resmi untuk membangun **plugin**, **konektor**, **ekstensi** dan **integrasi** untuk [Blueprin](blueprin-app.vercel.app) — platform anggaran arsitektural profesional untuk konstruksi di Indonesia.

## Mengapa Blueprin SDK?

Bangun ekstensi siap produksi untuk Blueprin tanpa mengimplementasikan ulang infrastruktur intinya.

- **Sistem Plugin** — Perluas Blueprin tanpa memodifikasi aplikasi host
- **Hooks & Events** — React terhadap event siklus hidup dan kustomisasi perilaku
- **Konektor** — Integrasi layanan dan pemasok eksternal
- **Formula Engine** — Perluas perhitungan RAB dengan aturan bisnis kustom
- **Komponen UI** — Bangun antarmuka plugin React yang terlihat native
- **Runtime Terisolasi** — Kendalikan eksekusi plugin dan akses jaringan
...
Atau buat plugin baru dengan CLI resmi:
...
Semua modul mendukung tree-shaking dan tersedia sebagai subpath import:
...
- **Sistem Plugin** — Daftar, aktifkan, nonaktifkan, dan kelola plugin dengan lifecycle hooks
- **Event Bus** — Sistem pub/sub untuk komunikasi antar plugin
- **Hook Registry** — Lifecycle hooks sebelum/sesudah untuk memperluas fungsionalitas
- **Storage Adapter** — Penyimpanan hybrid localStorage + Supabase dengan SSR guards
...
Plugin Blueprin berjalan di bawah izin yang dikendalikan oleh host.

| Kemampuan | Default | Kendali Host |
|---|---|---|
| Eksekusi plugin | Diaktifkan | Dikendalikan siklus hidup (aktifkan/nonaktifkan) |
| Jaringan eksternal | **Dinonaktifkan** | Diperlukan allowlist melalui manifest permissions |
| Hooks | Diaktifkan | Timeout 200ms per eksekusi hook |
| Penyimpanan | Terbatas | Dikendalikan adapter (localStorage atau Supabase) |
| Rendering UI | Diaktifkan | React sandbox, tanpa akses DOM langsung |

Plugin **tidak dapat** mengakses jaringan eksternal kecuali secara eksplisit di-whitelist oleh aplikasi host. Semua eksekusi hook memiliki timeout keras 200ms untuk mencegah infinite loop atau pemblokiran UI.
...
| [RAB Generator](example/rab_generator/) | Pembuatan RAB bertenaga AI |
| [WhatsApp Sync](example/whatsapp_sync/) | Notifikasi WhatsApp |
| [Material Connector](example/material_connector/) | Sinkronisasi pemasok |
| [Custom Report](example/custom_report/) | Pembuatan laporan |
...
- [Konektor](docs/connectors/)
- [Pengujian](docs/testing/)
- [Penerbitan](docs/publishing/)

## Referensi API

Dokumentasi API lengkap tersedia di [blueprin-docs.vercel.app/api](https://blueprin-docs.vercel.app/api):

- [BlueprinSDK](https://blueprin-docs.vercel.app/api/sdk) — Titik masuk utama
- [PluginManager](https://blueprin-docs.vercel.app/api/plugin-manager) — Siklus hidup plugin
- [EventBus](https://blueprin-docs.vercel.app/api/events) — Event pub/sub
- [HookRegistry](https://blueprin-docs.vercel.app/api/hooks) — Lifecycle hooks
- [Domain Clients](https://blueprin-docs.vercel.app/api/domain-clients) — Project, Material, RAB, dll.

## Berkontribusi

Kami menyambut kontribusi! Silakan lihat [Panduan Kontribusi](CONTRIBUTING.md) untuk detailnya.

## Komunitas

- [GitHub Discussions](https://github.com/qalvinahmad/blueprin-sdk/discussions) — Ajukan pertanyaan, bagikan ide, dan terhubung dengan pengembang lain
